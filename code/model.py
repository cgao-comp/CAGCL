...
class CAGCL(nn.Module):
    def __init__(self, des_size=768, tweet_size=768, num_prop_size=6, cat_prop_size=11, 
        ...
        des_dim = int(embedding_dimension * 0.25)
        tweet_dim = int(embedding_dimension * 0.25)
        num_prop_dim = int(embedding_dimension * 0.15)
        cat_prop_dim = int(embedding_dimension * 0.15)
        comm_dim = embedding_dimension - (des_dim + tweet_dim + num_prop_dim + cat_prop_dim)

        self.linear_relu_des = nn.Sequential(
            nn.Linear(des_size, des_dim),
            nn.BatchNorm1d(des_dim),
            nn.LeakyReLU()
        )
        ...
        print(f"Feature dimensions: description={des_dim}, tweet={tweet_dim}, numerical={num_prop_dim}, categorical={cat_prop_dim}, community={comm_dim}")
        print(f"Total embedding dimension: {des_dim + tweet_dim + num_prop_dim + cat_prop_dim + comm_dim}")
    ...
    def forward(self, des, tweet, num_prop, cat_prop, edge_index, edge_type, 
        ...
        x = torch.cat((d, t, n, c, comm), dim=1)
