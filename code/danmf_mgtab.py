...
from Dataset import Cresci15, MGTAB
import time
import os
...
def to_networkx_from_edge_list(edge_index, num_nodes=None, max_nodes=5000):
    ...
    if num_nodes > max_nodes:
        print(f"Number of nodes {num_nodes} exceeds max visualizable nodes {max_nodes}, sampling...")
...
def plot_graph(G, community_labels=None, sampled_indices=None, node_size=30, title="Graph"):
    ...
    plt.axis("off")
    plt.tight_layout()
    os.makedirs("outputs/mgtab", exist_ok=True)
    plt.savefig(f"outputs/mgtab/{title.replace(' ', '_')}.png", dpi=300)
    plt.close()
    print(f"Graph image saved to outputs/mgtab/{title.replace(' ', '_')}.png")
...
def evaluate_communities(edge_index, num_nodes, community_labels, danmf_model):
    ...
    if num_nodes > 10000:
        ...
        if len(all_nodes) < len(sampled_labels):
            print("Warning: Some nodes are not assigned to any community")
    ...
    print("Calculating intra- and inter-community densities...")
    intra_density = 0
    inter_density = 0
...
def save_community_data(community_labels, labels, train_idx, val_idx, test_idx):
    ...
    for comm_id in unique_communities:
        ...
        print(f"Processing community {comm_id}, containing {len(indices)} nodes")
    ...
    with open("outputs/mgtab/community_data.json", "w") as f:
        json.dump(community_data, f, indent=4)

    print("Community data saved to outputs/mgtab/community_data.json")
...
if __name__ == "__main__":
    ...
    dataset = MGTAB(device=device, process=False, save=False)
    tweets_tensor, num_prop, category_prop, edge_index, edge_type, labels, train_idx, val_idx, test_idx = dataset.dataloader1()

    num_nodes = tweets_tensor.size(0)
    ...
    model_path = "outputs/mgtab/danmf_model.pt"
    ...
    print("\nGetting community assignments...")
    community_labels = danmf_model.get_communities(n_clusters=2)
    np.save("outputs/mgtab/community_labels.npy", community_labels)
    print("Community labels saved to outputs/mgtab/community_labels.npy")
    ...
    for node_idx in range(num_nodes):
        if node_idx not in included_nodes:
            isolated_nodes.append(node_idx)
    ...
    print(f"Total nodes in all subgraphs: {total_nodes}")
    if total_nodes != num_nodes:
        print(f"Warning: Node count mismatch! ({total_nodes} vs {num_nodes})")
    else:
        print("Validation passed: all nodes are included in subgraphs")
    ...
    import json
    with open("outputs/mgtab/metrics.json", "w") as f:
    ...
    for i, subgraph_edge_index in enumerate(subgraph_edge_indices):
        if subgraph_edge_index.numel() > 0:
            np.save(f"outputs/mgtab/subgraph_{i}_edge_index.npy", subgraph_edge_index.cpu().numpy())
            print(f"Subgraph {i} edge list saved to outputs/mgtab/subgraph_{i}_edge_index.npy")
    ...
    print(f"\nDANMF community detection and subgraph splitting completed.")
