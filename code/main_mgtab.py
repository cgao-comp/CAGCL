...
def run_once(seed, device, args):
    ...
    total_nodes = tweets_tensor.size(0)

    community_file = "outputs/mgtab/community_labels.npy"
    ...
    for epoch in range(args['epochs']):
        ...
        output, contrastive_loss = model(
            tweets_tensor, num_prop, category_prop,
        ...
        if (epoch + 1) % 5 == 0:
            ...
            with torch.no_grad():
                ...
                if acc_val > best_val_acc:
                    ...
                    best_epoch = epoch + 1
                    patience_counter = 0
                    torch.save(model.state_dict(), "outputs/mgtab/best_CAGCL.pt")
                else:
                    patience_counter += 1
                if patience_counter >= args['early_stopping']:
                    break

    model.load_state_dict(torch.load("outputs/mgtab/best_CAGCL.pt"))
    model.eval()
    with torch.no_grad():
        test_output, _ = model(
            tweets_tensor, num_prop, category_prop,
...
def main():
    ...
    with open("outputs/mgtab/best_CAGCL_result.txt", "w") as f:
