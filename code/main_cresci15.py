...
def run_once(seed, device, args):
    ...
    community_file = "outputs/cresci15/community_labels.npy"
        ...
        if (epoch + 1) % 5 == 0:
            ...
            with torch.no_grad():
                ...
                if acc_val > best_val_acc:
                    ...
                    best_epoch = epoch + 1
                    patience_counter = 0
                    torch.save(model.state_dict(), "outputs/cresci15/best_CAGCL.pt")
                else:
                    patience_counter += 1
                if patience_counter >= args['early_stopping']:
                    break

    model.load_state_dict(torch.load("outputs/cresci15/best_CAGCL.pt"))
    model.eval()
    with torch.no_grad():
        test_output, _ = model(
            des_tensor, tweets_tensor, num_prop, category_prop,
...
def main():
    ...
    with open("outputs/cresci15/best_CAGCL_result.txt", "w") as f:
